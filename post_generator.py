...
# places the trademark at the bottom of the image (hardcoded placement)
def place_logo(bkg, trademark, font):
	bkg_width = 1080
	bkg_height = 1080
...
def build_image(im_path, quote, im_count='', logoify=True):
    ...
    # if the 'add trademark' option is selected then add the trademark
    if logoify:
        trademark = "@GreyMattersYT"
        tm_font = ImageFont.truetype("utils/tommy.otf", 52)
        tm_width, tm_height = draw.textsize(trademark, font=tm_font)
        draw.text(((W - tm_width)/2, H - 70 - tm_height), trademark, font=tm_font)
...
def main():
    ...
    logoify = (input("Include trademark? (y/n): ") == 'y')
